    ...
    while True:
        ...
        try:
            ...
            if stamp != cached_stamp:
                ...
                if cached_date == '':
                    with open(output_dir, 'r', encoding='utf8') as file_with_last_date:
                        line = None
                        for line in file_with_last_date:
                            pass
                        if line:
                            read_line = line
                        # read_line = file_with_last_date.readlines()
                        if line is not None:
                            group = re.search('(.*) CEF:0', read_line)
                            if group is not None:
                                cached_date = datetime.strptime(group.group(1), "%Y-%m-%d %H:%M:%S.%f")

                with open(mongo_log_path, 'r', encoding='utf8', errors='ignore') as lines:
